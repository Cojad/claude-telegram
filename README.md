...
| `send_sticker` | Send a sticker. Either `file_id` (already on Telegram servers) or `file` (absolute path to a local `.webp`/`.tgs`/`.webm` to upload directly), optionally `reply_to`. |
